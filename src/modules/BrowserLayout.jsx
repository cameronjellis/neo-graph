 ...
 * @updates:
 ****************************************************************************/
const url = "http://192.168.1.86:8085/query/12345";
...
class BrowserLayout extends React.Component {
  ...
  linkClick = (link, _event) => {
    link.source.serviceName = this.state.data.nodes.find(
      (x) => x.id === link.source.id
    ).properties.serviceName;
    link.target.serviceName = this.state.data.nodes.find(
      (x) => x.id === link.target.id
    ).properties.serviceName;
  ...
  getGraph = () => {
    if (!this.state.data)
      return <div className="graph-label">Submit a Cypher command</div>;
    else if (this.state.graphType === "TEXT")
      return <TextDisplay data={this.state.data} height={height} />;
    else if (this.state.graphType === "GRID")
      return <GridDisplay data={this.state.data} />;
    else if (this.state.graphType === "DAG")
      return <GridDisplay data={this.state.data} />;
    else if (this.state.graphType === "GRAPH")
      return (
        <Graph
